...
/// @ingroup solidfi_l1_compositions
/// @proposed
/// @brief Typed path finder. IS-A Converter<Graph, Path<T,U,P>, P>.
///
...
///
/// - **Solver<T,U,P>** — typed; finds a T->U route through a Graph. IS-A Converter.
...
/// to U? P is a routing signal: the same Graph may offer multiple T->U paths, and P
...
///
/// Because Solver IS-A Converter<Graph, Path<T,U,P>, P>, it composes: multiple Solver
...
/// - Solver MUST NOT modify the Graph passed to fetch().
...
class Solver : public Converter<Graph, Path<T, U, P>, P> {
public:
    /// @brief Find a path T->U through the given Graph. Returns it without executing.
    ///
    ...
    Path<T, U, P> fetch(Graph graph, P params) override;
};
